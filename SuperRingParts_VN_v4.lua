...
ScreenGui.IgnoreGuiInset = false  -- để Roblox tự chừa khoảng top bar
ScreenGui.DisplayOrder = 999       -- luôn nổi lên trên
...
local FULL_OFFSET = UDim2.new(0, 40, 0, 60)        -- neo top-left, cách mép 40px ngang, 60px dọc
local MINI_SIZE   = UDim2.new(0, 260, 0, 48)
local MINI_OFFSET = UDim2.new(0, 40, 0, 60)        -- thu nhỏ vẫn ở góc trên-trái

local MainFrame = Instance.new("Frame")
MainFrame.Name = "Main"
MainFrame.AnchorPoint = Vector2.new(0, 0)          -- neo góc trên-trái
...
UserInputService.InputChanged:Connect(function(input)
    if input == dragInput and dragging then
        ...
        -- Tính vị trí mới (giữ nguyên Scale, chỉ thay đổi Offset)
        local newX = startPos.X.Offset + d.X
        local newY = startPos.Y.Offset + d.Y

        -- Giới hạn trong màn hình để menu không bị kéo ra ngoài
        local viewport = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize
                      or Vector2.new(1280, 720)
        local maxX = math.max(0, viewport.X - MainFrame.AbsoluteSize.X)
        local maxY = math.max(0, viewport.Y - MainFrame.AbsoluteSize.Y)
